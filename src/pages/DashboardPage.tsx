          ...
          {selectedDevice ? <div className="space-y-4 h-full">
              ...
              {selectedDevice && <Card className="bg-white rounded-2xl shadow-sm border-0">
                  <CardHeader className="pb-2">
                    <CardTitle className="text-gray-900 text-lg">Previous Work Documents</CardTitle>
                    <p className="text-sm text-gray-500">Employee: {selectedDevice.assignedPerson || 'Unassigned'} | Device: {selectedDevice.deviceName}</p>
                  </CardHeader>
                  <CardContent className="p-4 pt-0">
                    <div className="space-y-3">
                      {selectedDevice.previousWork.map(work => <div key={work.id} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-gray-300 hover:shadow-sm transition-all duration-200">
                          <div className="flex items-start space-x-4 flex-1">
                            <div className="w-10 h-10 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl flex items-center justify-center border border-blue-200">
                              <FileText className="w-5 h-5 text-blue-600" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  <h4 className="font-semibold text-gray-900 text-base mb-1">{work.title}</h4>
                                  <p className="text-sm text-gray-600 mb-2 line-clamp-2">{work.description}</p>
                          ...
                          <Button onClick={() => handleDownload(work)} size="sm" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg ml-4 flex-shrink-0">
                            <Download className="w-3 h-3 mr-2" />
                            Download
                          </Button>
                        </div>)}
                      {selectedDevice.previousWork.length === 0 && <div className="text-center py-12 text-gray-500 bg-gray-50 rounded-xl">
                          <FileText className="w-16 h-16 mx-auto mb-3 opacity-40" />
                          <h3 className="text-lg font-medium text-gray-700 mb-1">No Work Documents</h3>
                          <p className="text-sm">No previous work records found for {selectedDevice.assignedPerson || selectedDevice.deviceName}</p>
